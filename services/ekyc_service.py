...
# # from .gender_detection_service import gender_detection_service
# from models.liveness_detection.silent_anti_spoofing import SilentAntiSpoofing
# from utils.image_utils import extract_face_box
# from models import (
#     MTCNN,
#     VGGFace2,
...
#                     return {
#                         'success': False,
#                         'error': {'message': f'Invalid base64 image: {str(e)}'},
#                         'debug_info': None
#                     }

#             # Anti-spoofing check with visualization
#             is_real, spoof_score, debug_image = self.anti_spoofing.detect_with_visualization(frame)
            
#             # Convert numpy values to Python native types
#             spoof_score = float(spoof_score)
#             is_real = bool(is_real)
#             image_shape = [int(x) for x in frame.shape]
...
#                 debug_info['error_type'] = 'face_detection_failed'
#                 return {
#                     'success': False, 
...
#             if not challenge:
#                 verification_result = self._handle_verification(frame, boxes, user_id)
#                 verification_result['debug_info'] = debug_info
#                 return verification_result

#             # For challenge mode (with challenge)
#             orientation = self.orientation_detector.detect(landmarks[0])
#             debug_info['detected_orientation'] = str(orientation)
            
#             # Convert orientation check to Python bool
#             is_challenge_passed = bool(orientation == challenge)
            
#             result = {
#                 'success': is_challenge_passed,
#                 'result': {
#                     'message': 'Challenge passed' if is_challenge_passed else 'Challenge failed'
#                 },
#                 'debug_info': debug_info
#             }
            
#             return result if is_challenge_passed else {
#                 'success': False,
#                 'error': {
...
#             return {'success': False, 'error': {'message': 'Failed to extract face features'}}
...
#                     'mean_similarity': verification_result.get('mean_similarity', 0),
#                     'max_similarity': verification_result.get('max_similarity', 0)
#                 }
#             }
...
#             'success': False,
#             'error': {
#                 'message': verification_result.get('error', 'Face verification failed'),
...
#             face_pil = face_pil.resize((IMAGE_SIZE, IMAGE_SIZE), Image.Resampling.LANCZOS)
            
#             # Convert to tensor and normalize
#             face_tensor = torch.from_numpy(np.array(face_pil)).permute(2, 0, 1).float()
#             face_tensor = face_tensor.div(255.0).unsqueeze(0).to(self.device)
            
#             # Extract features
#             with torch.no_grad():
#                 features = self.verification_model(face_tensor)
...
#         try:
#             if not face_features: 
#                 return {"verified": False, "error": "No stored face features"}
#             if face_frame_feature is None: 
#                 return {"verified": False, "error": "Invalid input feature"}
...
#                 if norm > 1e-10:
#                     normalized_features.append(f / norm)
            
#             if not normalized_features: 
#                 return {"verified": False, "error": "Invalid stored features"}
            
#             # Calculate similarities
#             stored_array = np.vstack(normalized_features)
#             similarities = np.dot(stored_array, face_frame_feature)
...
#         except Exception as e:
#             return {"verified": False, "error": f"Matching error: {str(e)}"}
...
#             if not is_real:
#                 return { 'success': False,'error': {'message': 'Spoof detected', 'score': spoof_score}}
#             face_feature = self.extract_face_features(frame)
#             if face_feature is None:
#                 return { 'success': False, 'error': { 'message': 'Failed to extract face features' } }
#             similar_faces = db_service.search_similar_faces(face_feature, k=top_k)
#             if not similar_faces:
#                 return {'success': False,'error': { 'message': 'No similar faces found' } }
#             # Định dạng kết quả trả về
#             results = []
#             for user_id, similarity in similar_faces:
#                 results.append({
#                     'user_id': user_id,
